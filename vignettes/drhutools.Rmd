...
If `type = "polygon"`, the data frame should follow the exemplary structure (`toy_poly`) below:
...
With the well-organized data, users can easily draw a national map of China at either provincial or prefectural levels:^[If users encounter errors or unreadable output, adjusting the encoding may help.]
...
Apart from polygonous maps, `goodmap` can also draw points on a national map.
To do so, users need to set `type = "point"`.
And the data frame should follow the exemplary structure below:
...
In this data structure, users can use `g_lat` and `g_lon` to set up the latitudes and longitudes of the points and vary the column.
The toy_point$value_set is discrete variables, so the color_type choose `factor`.
Users can name the `legend_name` as they wish.
...
`goodmap` can also create animations to illustrate specified geographic dynamics. 
Users simply set `animate = TRUE` and specify the time variable for the function to draw the map accordingly.
Here is an example:
...
In the current version, the animated plot will be stored in a temporary file.
If users are satisfied with the output, they need to save it into wherever they want before rerun the function again.
