...
if(!(default_locate == "English_United States.utf8"))
  Sys.setlocale(category = "LC_CTYPE", locate = "English_United States.utf8")
```
...
goodmap(toy_map, type = "polygon", level = "province", animate = TRUE, animate_var = "year_set")
```


If the map type is `polygon`, the dataframe should follow the structure below:

| id | city       | pro       | year_set | variable   |
|----|------------------------|----------|------------|
| 1  | 乌鲁木齐  | 新疆       | 2011     | 0.2861395  |
| 2  | 拉萨      | 西藏       | 2011     | 0.3881083  |
| 3  | 呼和浩特  | 内蒙       | 2011     | 0.9466682  |
| 4  | 西宁      | 青海       | 2012     | 0.8360043  |
| 5  | 成都      | 四川       | 2012     | 0.4622928  |
| 6  | 哈尔滨    | 黑龙江     | 2012     | 0.1387102  |
...
|---------------------------|----------|---------|----------|-------|--------|------|
| 中国共产党历史展览馆      | 2021     | 40.00379| 116.3994 | 北京  | 北京   | 7    |
| 中央礼品文物管理中心      | 2021     | 39.89785| 116.4109 | 北京  | 北京   | 7    |
| 中国美术馆                | 2021     | 39.92535| 116.4090 | 北京  | 北京   | 8    |
| 中国电影博物馆            | 2021     | 39.99636| 116.5206 | 北京  | 北京   | 8    |
| 中国邮政邮票博物馆        | 2021     | 39.91081| 116.4311 | 北京  | 北京   | 8    |
| 中国钱币博物馆            | 2021     | 39.90215| 116.3949 | 北京  | 北京   | 8    |
...
goodmap(data_file, type = "point", animate = TRUE, animate_var = "year_set", custom_colors = "pink",base_radius = 1, radius_factor = 1)
```

The function returns the file path of the saved maps GIF. The map is also saved as an HTML file for interactive viewing.

### Figure: Animation for all panel data
```{r out.width = '70%', echo = FALSE}
